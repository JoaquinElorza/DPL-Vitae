...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-2xl shadow-md overflow-hidden">
        ...
        {{-- Formulario --}}
        <form action="{{ route('servicios.update', $servicio->id_servicio) }}" method="POST" class="px-8 py-8 space-y-6">
            @csrf
            @method('PUT')
                    ...
                    <select name="id_ambulancia"
                        ...
                        @foreach($ambulancias as $ambulancia)
                            <option value="{{ $ambulancia->id_ambulancia }}"
                                {{ old('id_ambulancia', $servicio->id_ambulancia) == $ambulancia->id_ambulancia ? 'selected' : '' }}>
                    ...
                    <select name="id_cliente"
                        ...
                        @foreach($clientes as $cliente)
                            <option value="{{ $cliente->id_cliente }}"
                                {{ old('id_cliente', $servicio->id_cliente) == $cliente->id_cliente ? 'selected' : '' }}>
                                #{{ $cliente->id_cliente }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="datetime-local" name="fecha_hora"
                           value="{{ old('fecha_hora', \Carbon\Carbon::parse($servicio->fecha_hora)->format('Y-m-d\TH:i')) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="costo_total"
                               value="{{ old('costo_total', $servicio->costo_total) }}"
                               step="0.01" min="0"
                ...
                <div>
                    ...
                    <select name="estado"
                        ...
                        @foreach(['Programado', 'En curso', 'Finalizado', 'Cancelado'] as $estado)
                            <option value="{{ $estado }}"
                                {{ old('estado', $servicio->estado) == $estado ? 'selected' : '' }}>
                                {{ $estado }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <textarea name="observaciones" rows="4"
                          ...
                          class="w-full rounded-2xl bg-[#e9e9e9] px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-[#7aa6c2] resize-none @error('observaciones') ring-2 ring-red-400 @enderror">{{ old('observaciones', $servicio->observaciones) }}</textarea>
                @error('observaciones')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
